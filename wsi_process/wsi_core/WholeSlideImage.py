...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def get_seg_mask(self, region_size, scale, use_holes=False, offset=(0,0)):
        ...
        # thumbnail_gray =  cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        # edges = filters.sobel(thumbnail_gray)
        ...
        # mask = np.array(hyst+hight, np.uint8)
